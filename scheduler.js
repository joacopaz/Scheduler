...
Spots.find(e => parseInt(e.Break.slice(3, 5)) === 23 || parseInt(e.Break.slice(3, 5)) === 48 || parseInt(e.Break.slice(3, 5)) === 49) ? diaMediaHora = true : diaMediaHora = false;
if (diaMediaHora) {
    dia = [{
            Hora: 7,
            Remaining: 60,
...
spotsPendientes.forEach(spot => {
    let indice = 0;
    if (breaks[spot.Hora].Spots.find(e => e.Slot === spot.Ubicacion) && spot.Asignado === false) {
...
dia.reduce((a, e) => a + e.Spots.length, 0) === Spots.length ? console.log('Todos los spots fueron insertados.') : console.log('Hubo un problema, no se insertaron todos los spots.')

const diaEnArray = dia.map(dia => {
    if (dia.Spots.length === 0) {
        return [dia.Hora]
    } else if (dia.Spots.length === 1) {
        return [dia.Hora, dia.Spots[0].Slot, dia.Spots[0].Media, dia.Spots[0].Version, dia.Spots[0].Duracion]
    } else if (dia.Spots.length > 1) {
        const array = []
        for (let i = 0; i < dia.Spots.length; i++) {
            const element = dia.Spots[i];
            array.push([dia.Hora, element.Slot, element.Media, element.Version, element.Duracion])
        }
        return array;
    }
})
...
const diaOrdenado = [
    [''],
    ['', 'Media', 'Hora', 'Pos', 'Version', 'Duracion'],
    [''],
]
    ...
    diaOrdenado.forEach((e, i) => {
        if (e.length > 1 && e[2] !== undefined) {
            if (e[2].toString().includes('.5'))
                e[2] = e[2].toString().slice(0, e[2].toString().lastIndexOf(':30') - 1) + ':30'
        }
    })
}
...
diaOrdenado.forEach((e, i) => {
    if (e[1] === 'Promotion' && e[5] !== 30 && typeof (diaOrdenado[i - 1]) !== 'undefined' && diaOrdenado[i - 1][1] === 'Promotion') {
        let indexPrimeraPromo = diaOrdenado.findIndex(x => x[2] === e[2] && x[1] === 'Promotion')
        let duracion = e[5]
...
diaOrdenado.forEach((e, i) => {
    if (e[5] === 25 && e[1] === 'Promotion') {
        ...
        diaOrdenado.splice(++ubicacion, 0, ['', 'Promotion', e[2], ++slot, 'Promotion', 5])
...
if (answer === true) {
    ...
    FS.writeFileSync('./Orden' + ' ' + canal + ' ' + fechasOrd[0] + '.xlsx', buffer)

    console.log('Finalizada la escritura de la Orden ' + canal + ' ' + fechasOrd[0] + '.xlsx');
...
diaEnArray.push(fechasOrd[0])
diaEnArray.push(diaMediaHora)

module.exports = diaEnArray;

Spots.sort((a, b) => a.Version.localeCompare(b.Version, 'en', {
    numeric: true
}))
