...
if (answer === true) {
    ...
    if (canal.length > 100) {
        canal = (nuevo.slice(nuevo.indexOf('ITX') + 4, nuevo.indexOf('ITX') + 13)).trim()
    ...
    nuevo.forEach((e, i) => {
        if (i === 0) // Se crea una exepción para el primer spot ya que necesita info previa del indice anterior, cosa que el primero no tiene
        {
            spots.push({
                Spot: spotId++,
                Hora: parseInt(nuevo[0].slice(nuevo.lastIndexOf('0') - 11, nuevo[0].lastIndexOf('0') - 8)),
                Minutos: nuevo[0].slice(nuevo.lastIndexOf('0') - 8, nuevo[0].lastIndexOf('0') - 5),
                Media: nuevo[1].slice(nuevo[1].indexOf('SKSA'), nuevo[1].indexOf('SKSA') + 14),
                Version: (nuevo[1].slice(nuevo[1].indexOf('SKSA') - 80, nuevo[1].indexOf('Used:') - 1)).trim(),
                Duracion: nuevo[1].slice(nuevo[1].indexOf('0') + 3, nuevo[1].indexOf('0') + 8),
            })
        } else {
            spots.push({
                Spot: spotId++,
                Hora: parseInt(nuevo[i - 1].slice(nuevo.lastIndexOf('0') - 11, nuevo[i - 1].lastIndexOf('0') - 8)),
                Minutos: parseInt(nuevo[i - 1].slice(nuevo.lastIndexOf('0') - 8, nuevo[i - 1].lastIndexOf('0') - 5)),
                Media: nuevo[i].slice(nuevo[i].indexOf('SKSA'), nuevo[i].indexOf('SKSA') + 14),
                Version: (nuevo[i].slice(nuevo[1].indexOf('SKSA') - 80, nuevo[i].indexOf('Used:') - 1)).trim(),
                Duracion: nuevo[i].slice(nuevo[i].indexOf('0') + 3, nuevo[i].indexOf('0') + 8),
            })
        }
    })
    spots.splice(0, 1)
    ...
    const resultado = []
    const logeo = []
    resultado.push([`El canal de la orden es ${canalOrden} y en IBMS se insertó en ${canal}.`])
    logeo.push([''], ['',
        'Canal', 'Orden: ' + canalOrden, 'IBMS: ' + canal
    ])
    resultado.push([`\nLa fecha de la orden es ${diaOrden} y en IBMS se insertó el ${fecha}.`])
    logeo.push(['',
        'Fecha', 'Orden: ' + diaOrden, 'IBMS: ' + fecha
    ])
    ...
    if (spots.length === diaFinal.length) {
        ...
        resultado.push(['\nHay ' + spots.length + ' Spots insertados, debería haber ' + diaFinal.length])
        logeo.push(['',
            'Conteo:', spots.length + '/' + diaFinal.length, 'Orden/IBMS'
        ], [''])
    }
    ...
    spots.forEach((e, i) => {
        if (diaFinal.find(x => e.Hora === x.Hora && e.Media === x.Media && e.Encontrado === false)) {

            if (typeof diaFinal.find(x => e.Hora === x.Hora && e.Media === x.Media && x.Insertado === false) !== 'undefined') {
                diaFinal.find(x => e.Hora === x.Hora && e.Media === x.Media && x.Insertado === false).Insertado = true;
                spots.find(x => e.Hora === x.Hora && e.Media === x.Media && x.Encontrado === false).Encontrado = true;
            }
        }
    })
    ...
    if (spots.filter(e => e.Encontrado === false).length < 1) {
        ...
        spots.filter(e => e.Encontrado === false).forEach(e => {
            resultado.push(['\nRevisar: A las ' + e.Hora + ' horas, el media ID : ' + e.Media + ' ' + e.Version]);
            logeo.push(['',
                'Revisar:', e.Media, e.Version, 'Hora: ' + e.Hora
            ])
        })
    }
    ...
    if (diaFinal.filter(e => e.Insertado === false).length < 1) {
        ...
        diaFinal.filter(e => e.Insertado === false).forEach(e => {
            resultado.push(['\nFaltante!! A las ' + e.Hora + ' horas en el slot ' + e.Slot + ' falta el media ID : ' + e.Media + ' ' + e.Version])
            logeo.push(['',
                'Missing:', e.Media, e.Version, 'Hora: ' + e.Hora, 'Slot: ' + e.Slot
            ])
        })
    }
    ...
    if (answer === true) {
        ...
        if (logeo.length === 3) {
            ...
            fs.writeFileSync('./Checkeo_' + canal + '_' + fecha.replace(/\//ig, '-') + '.txt', escritura, 'utf-8')

            console.log('Se escribió el chequeo en Checkeo_' + canal + '_' + fecha.replace(/\//ig, '-') + '.txt.')
        }
        ...
        if (logeo.length !== 3) {
            ...
            fs.writeFileSync('./Checkeo' + ' ' + canal + ' ' + fecha.replace(/\//ig, '-') + '.xlsx', buffer)

            console.log('Se escribió el chequeo en Checkeo_' + canal + '_' + fecha.replace(/\//ig, '-') + '.xlsx.')
        }
