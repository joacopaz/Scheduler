...
function extraerCliente() {
    if (typeof (search('Cliente')) !== "undefined" && typeof (search('Cliente')[0]) !== 'undefined') {
...
rawIzzi != undefined && rawIzzi !== [] ? izzis = rawIzzi.map(e => {
    return {
        Canal: e[0].trim(),
        Hora: parseInt(e[1]),
        Corte: parseInt(e[2]),
...
Izzi !== [] ?
    Izzi.forEach(izzi => {
        Spots.forEach(spot => {
            if (spot.Hora == izzi.Hora && spot.Ubicacion == izzi.Posicion && izzi.MediaId == spot.Media) {
                spot.Izzi = true;
                izzi.SpotId = spot.Id;
            }
        })
